...
RUN mkdir -p ./packages/anki
COPY . ./packages/anki
WORKDIR ./packages/anki
...
WORKDIR /dist
COPY --from=build-deps /js-base-image/packages/anki/dist /dist
COPY --from=build-deps /js-base-image/packages/anki/resource/ /dist/
